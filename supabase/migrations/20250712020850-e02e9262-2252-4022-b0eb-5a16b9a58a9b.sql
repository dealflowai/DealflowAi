...
BEGIN
  ...
  -- Ensure tokens are reset for current month
  PERFORM reset_monthly_tokens(p_user_id);
...
-- Create a function to handle monthly reset for all users (for cron job)
CREATE OR REPLACE FUNCTION public.reset_all_monthly_tokens()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  user_record RECORD;
BEGIN
  -- Reset tokens for all users who haven't been reset this month
  FOR user_record IN 
    SELECT DISTINCT ut.user_id, p.plan_tokens
    FROM user_tokens ut
    JOIN profiles p ON p.id = ut.user_id
    WHERE ut.monthly_reset_date < DATE_TRUNC('month', CURRENT_DATE)
       OR ut.monthly_reset_date IS NULL
  LOOP
    PERFORM reset_monthly_tokens(user_record.user_id);
...
-- Set up monthly cron job to reset tokens on the 1st of each month
SELECT cron.schedule(
  'monthly-token-reset',
  '0 0 1 * *', -- At 00:00 on the 1st day of every month
  $$
  SELECT reset_all_monthly_tokens();
  $$
);