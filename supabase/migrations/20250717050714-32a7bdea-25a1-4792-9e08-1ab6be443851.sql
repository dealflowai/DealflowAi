-- Fix the duplicate detection trigger to not compare with NEW.id which doesn't exist yet
DROP TRIGGER IF EXISTS trigger_detect_duplicates ON public.buyers;

-- Update the function to not check NEW.id since it doesn't exist in BEFORE INSERT
...
-- Recreate trigger for automatic duplicate detection on buyer insert
